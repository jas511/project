...
class Newspaper():
    def __init__(self,frame):
        ...
        self.col = ['newspapername','authorname','location']
        self.tree = ttk.Treeview(self.content,columns=self.col,show='headings',height=18)
        self.tree.heading('newspapername',text='Newspaper Name')
        self.tree.column('newspapername',anchor=CENTER,width=300)
        ...
        try:
            result = dbms.fetchnewspaperdetails()
            for i in result:
        ...
        self.title = Label(self.content,text="All Newspapers are listed here",font="Arial 25 bold",bg="white")
        self.title.place(x=350,y=20)

        self.newspapernamelbl = Label(self.content,text="Enter Newspaper Name",font="Arial 12",bg="white")
        self.newspapernamelbl.place(x=25,y=100)
        self.newspapernameinp = Entry(self.content,width=12,font="Arial 10 bold",bd=3)
        self.newspapernameinp.place(x=220,y=100)
        ...
        self.enterdetails = Button(self.content,text="Enter Newspaper Details",bg="#56a9e8",fg="white",font="Arial 10 bold",command=self.enterdetailfn)
        self.enterdetails.place(x=910,y=150)
    
    def enterdetailfn(self):
        if not(self.newspapernameinp.get()) or not(self.authornameinp.get()) or not(self.locationinp.get()):
            messagebox.showwarning("Warning","Please enter all the fields")
                ...
                if res:
                    details = (self.newspapernameinp.get(),self.authornameinp.get(),self.locationinp.get())
                    result = dbms.enternewspaperdetail(details)
                if result:
